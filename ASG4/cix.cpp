...
void cix_get (client_socket& server, string filename) {
   ...
   if (header.command != cix_command::FILEOUT) {
      ...
      outlog << "server returned " << header << endl;
      outlog << filename << " was unsuccessfully saved" << endl;
   } else {
      size_t host_nbytes = ntohl (header.nbytes);
      auto buffer = make_unique<char[]> (host_nbytes + 1);
      recv_packet (server, buffer.get(), host_nbytes);
      ...
      out.write (buffer.get(), host_nbytes - 1);
      out.close();
      outlog << filename << " was saved successfully" << endl;
   }
}

void cix_put (client_socket& server, string filename) {
   ...
   if (!file) {
      outlog << "put " << header.filename << " failed: "
         << strerror(errno) << endl;
   } else {
      file.seekg (0, file.end);
      int file_length = file.tellg();
      file.seekg(0, file.beg);
      auto buffer = make_unique<char[]> (file_length + 1);
      buffer[file_length] = '\0';
      file.read (buffer.get(), file_length);
      header.command = cix_command::PUT;
      header.nbytes = htonl(file_length + 1);
      outlog << "sending header " << header << endl;
      send_packet (server, &header, sizeof header);
      send_packet (server, buffer.get(), file_length + 1);
      recv_packet (server, &header, sizeof header);
      outlog << "received header " << header << endl;
   }
   if (header.command == cix_command::ACK) {
      outlog << filename << " successfully put on server" << endl;
   } else if (header.command == cix_command::NAK) {
      outlog << "Failed to put " << filename <<  " on server" << endl;
   }
}
...
void cix_rm (client_socket& server, string filename) {
   ...
   if (header.command != cix_command::ACK) {
      ...
      outlog << "server returned " << header << endl;
      outlog << filename << " cannot be removed" << endl;
   } else {
      outlog << filename << " removed successfully" << endl;
   }
}
